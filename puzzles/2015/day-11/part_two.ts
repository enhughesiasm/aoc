...
export async function solve(dataPath?: string) {
  ...
  const PASSWORD_TO_FIND = 2; // find the second new password

  for (let i = 0; i < PASSWORD_TO_FIND; i++) {
...
function findNextPassword(pw: string): string {
  let password = pw;
  ...
  do {
    ...
    isPasswordValid =
      includesValidStraight(password) && containsTwoDifferentPairs(password);
