...
export async function solve(dataPath?: string) {
  ...
  return findMaximumGoldSpendForLoss(player, boss);
}

function findMaximumGoldSpendForLoss(player: Char, boss: Char) {
  let maximumGoldSpend = Number.NEGATIVE_INFINITY;
      ...
      for (let ringOneI = -1; ringOneI < RINGS.length; ringOneI++) {
        for (let ringTwoI = -1; ringTwoI < RINGS.length; ringTwoI++) {
          ...
          if (!doesPlayerWin(playerWithLoadout, thisBoss)) {
            maximumGoldSpend = Math.max(maximumGoldSpend, spentThisRound);
          }
        }
      }
    }
