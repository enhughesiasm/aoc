...
function findLeastManaSpendToWin(state: State, global: GlobalState) {
  state.player.hp -= 1; // hard mode so lose 1 hp at start of first turn
...
function play(s: State, nextSpell: Spell, global: GlobalState, depth: number) {
  ...
  const resultingState = playRound(state, nextSpell);
...
function playRound(state: State, spell: Spell): State {
  ...
  // PART TWO - Remove one HP per turn
  const stateAfterHardMode = structuredClone(stateAfterBossTurn);
  stateAfterHardMode.player.hp -= 1;
  if (isPlayerDead(stateAfterHardMode)) return stateAfterHardMode;

  // EFFECTS
  const endState = applyEffects(stateAfterHardMode);
